...
def reload_env():
    ...
    # 清除旧的环境变量
    if 'FREE11_API_KEY' in os.environ:
        del os.environ['FREE11_API_KEY']
    ...
    if not API_KEY:
        raise ValueError("FREE11_API_KEY not found in .env file")
...
if not API_KEY and not TEST_MODE:
    raise ValueError("FREE11_API_KEY not found in .env file and TEST_MODE is not enabled")
...
# 白山智算 API URL
BAISHAN_API_URL = "https://api.edgefn.net/v1/chat/completions"
...
def chat_completions():
    ...
    try:
        ...
        # 白山智算 API 已经是 OpenAI 兼容格式，直接返回
        response_data = {
            "id": result.get("id", ""),
            "object": "chat.completion",
            "created": result.get("created", 0),
            "model": result.get("model", "DeepSeek-R1-0528-Qwen3-8B"),
            "choices": result.get("choices", []),
            "usage": result.get("usage", {}),
        }
        
        # 如果 content 为空但有 reasoning_content,则将 reasoning_content 复制到 content
...
def execute_with_retry(data, message_id):
    ...
    # 获取请求的模型，默认使用 DeepSeek-R1-0528-Qwen3-8B
    requested_model = data.get("model", "DeepSeek-R1-0528-Qwen3-8B")
    
    for attempt in range(max_retries):
        try:
            # 构建白山智算请求（OpenAI 兼容格式）
            baishan_payload = {
            ...
            if TEST_MODE:
                ...
                print(f"[请求] 发送到白山智算 API [{requested_model}] {attempt_str} [超时: {current_timeout}s]")
                
                response = session.post(
                    BAISHAN_API_URL,
                    json=baishan_payload,
                    headers=headers,
                    proxies=proxies,
        ...
        <div class="container">
            ...
            <div id="chat-tab" class="tab-content">
                ...
                <div style="margin-bottom: 15px; padding: 10px; background-color: #f0f8ff; border-radius: 5px; font-size: 13px; color: #666;">
                    ...
                    <br><strong>📍 可用模型:</strong> DeepSeek-R1-0528-Qwen3-8B, KAT-Coder-Exp-72B-1010, MiniMax-M2.5
                </div>
                <div style="margin-bottom: 10px;">
                    <label for="modelSelect" style="font-weight: bold; color: #333;">模型:</label>
                    <select id="modelSelect" style="padding: 5px; border: 1px solid #ddd; border-radius: 4px; margin-left: 10px; min-width: 200px;">
                        <option value="DeepSeek-R1-0528-Qwen3-8B">DeepSeek-R1-0528-Qwen3-8B</option>
                        <option value="KAT-Coder-Exp-72B-1010">KAT-Coder-Exp-72B-1010</option>
        ...
        <script>
            ...
            function sendMessage() {
                ...
                fetch('/v1/chat/completions', {
                    ...
                    body: JSON.stringify({
                        model: modelSelect.value, // 使用选择的模型
                        messages: [
            ...
            // 初始化聊天界面
            document.getElementById('chatMessages').innerHTML = '<div class="message assistant">欢迎使用白山智算 AI 聊天测试！您可以在这里直接测试代理功能。支持模型：DeepSeek-R1-0528-Qwen3-8B, KAT-Coder-Exp-72B-1010, MiniMax-M2.5</div>';
        </script>
    </body>
    </html>
    """
